...
corrfile = "MECTADgraphPt0175cut100kwindow_HeatmapCorrelations.txt"
...
# get correlations between columns for abvalsarray TPD
colcombs = combinations(range(len(cases)), 2)
corrlist_tpd = []
for col1, col2 in colcombs:
    corr = np.corrcoef(abvalsarray[:,col1], abvalsarray[:,col2])
    writestring = cases[col1] + " & " + cases[col2] + " : " + str(corr) + "\n"
    corrlist_tpd.append(writestring)
...
# get correlations between columns for abvalsarray MCL
colcombs = combinations([0,1], 2)
corrlist_mcl = []
for col1, col2 in colcombs:
    corr = np.corrcoef(abvalsarray[:,col1], abvalsarray[:,col2])
    writestring = cases[col1] + " & " + cases[col2] + " : " + str(corr) + "\n"
    corrlist_mcl.append(writestring)
    ...
    #
    # #fig.tight_layout()
    # newplotname = plot3d.split(".")[0] + "_plot" + str(namenum) + "." + plot3d.split(".")[1]
...
# get correlations between columns for abvalsarray spectral
colcombs = combinations(range(len(cases)), 2)
corrlist_spectral = []
for col1, col2 in colcombs:
    corr = np.corrcoef(abvalsarray[:,col1], abvalsarray[:,col2])
    writestring = cases[col1] + " & " + cases[col2] + " : " + str(corr) + "\n"
    corrlist_spectral.append(writestring)
    ...
    #
    # #fig.tight_layout()
    # newplotname = plot4d.split(".")[0] + "_plot" + str(namenum) + "." + plot4d.split(".")[1]
    # plt.savefig(newplotname)
    # plt.clf()
    #
    # i1 = i1 + numprint
...
with open(corrfile, "w") as cf:
    cf.write("TPD Correlations:\n")
    for i in corrlist_tpd:
        cf.write(i)
    cf.write("\n")

    cf.write("MCL Correlations:\n")
    for i in corrlist_mcl:
        cf.write(i)
    cf.write("\n")

    cf.write("Spectral Correlations:\n")
    for i in corrlist_spectral:
        cf.write(i)
    cf.write("\n")
